...
function App() {
  ...
  return (
    <div style={{minHeight:"100vh",background:C.bg,color:C.text,fontFamily:"'Inter','Segoe UI','Segoe UI Emoji','Apple Color Emoji','Noto Color Emoji',sans-serif"}}>
      <style>{`
        ...
        {data && !laden && (
          <div>
            ...
            <div style={S.card(C.muted)}>
              ...
              <BijlageUploader bestanden={notitiesBestanden} setBestanden={setNotitiesBestanden} label="Ã°ÂÂÂ Foto of bijlage toevoegen (optioneel)"/>
          ...
          return(
            <div key={h.id} className="card" style={{borderTop:`3px solid ${hasIssue?"#ef4444":hasVrij?C.blauw:C.groen}`,padding:"18px 18px 14px"}}>
              ...
              <div style={{display:"flex",gap:4,flexWrap:"wrap",marginBottom:12}}>
                {[...h.kamers].sort((a,b)=>parseFloat(a.k)-parseFloat(b.k)).map(k=>{const c=STATUS_MAP[k.status]||{dot:C.muted};return <div key={k.k} title={`K${k.k}: ${k.naam||"leeg"} Ã¢ÂÂ ${k.status}`} style={{width:12,height:12,borderRadius:3,background:c.dot+"50",border:`1.5px solid ${c.dot}`}}/>;  })}
              </div>
